...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof saxpb, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function multiplies each element by a scalar constant and adds a scalar constant', function test( t ) {
	...
	actual = saxpb( [ x, alpha, beta ] );
	t.strictEqual( actual, x, 'returns expected value' );

	expected = new Float32Array( [ -7.0, 8.0, 18.0, -22.0, 23.0 ] );
...
tape( 'the function supports an input ndarray having a non-unit stride', function test( t ) {
	...
	xbuf = new Float32Array( [ -2.0, 1.0, 3.0, -5.0, 4.0, 0.0, -1.0, -3.0 ] );
	x = vector( xbuf, 4, 2, 0 );
	alpha = scalar2ndarray( 5.0, {
		'dtype': 'float32'
	});
	...
	expected = new Float32Array( [ -7.0, 1.0, 18.0, -5.0, 23.0, 0.0, -2.0, -3.0 ] );
	t.deepEqual( xbuf, expected, 'returns expected value' );
...
tape( 'the function supports an input ndarray having a non-zero offset', function test( t ) {
	...
	xbuf = new Float32Array( [ 1.0, -2.0, 3.0, -5.0, 4.0, 0.0 ] );
	x = vector( xbuf, 3, 1, 2 );
	alpha = scalar2ndarray( 5.0, {
		'dtype': 'float32'
	});
	...
	expected = new Float32Array( [ 1.0, -2.0, 18.0, -22.0, 23.0, 0.0 ] );
	t.deepEqual( xbuf, expected, 'returns expected value' );
